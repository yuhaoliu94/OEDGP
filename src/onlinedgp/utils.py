...
from src.onlinedgp.datasets import DataSet
...
def import_dataset(data_name: str, data_fold: int) -> DataSet:
    ...
    X_path = os.path.join(cwd, "folds", "%s_fold_%s_X.txt" % (data_name, data_fold))
    X = np.loadtxt(X_path, delimiter=' ')

    Z_path = os.path.join(cwd, "folds", "%s_fold_%s_Z.pickle" % (data_name, data_fold))
    if os.path.exists(Z_path):
        with open(Z_path, 'rb') as f:
            Z = pickle.load(f)
        data = DataSet(data_name, data_fold, Y, X, Z)
    else:
