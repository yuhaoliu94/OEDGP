...
class DgpRff(object):
    def __init__(self, likelihood_fun, num_examples, d_in, d_out, n_layers, n_rff, df, kernel_type,
                 kernel_arccosine_degree, is_ard, local_reparam, feed_forward, q_Omega_fixed, theta_fixed, learn_Omega):
        """
        ...
        self.loss, self.kl, self.ell, self.layer_out, self.layer, self.omega_from_q = self.get_nelbo()
    ...
    def init_posterior_Omega(self):
        ...
        mean_Omega = [tf.Variable(mu[i] * tf.ones([self.d_in[i], self.d_out[i]]), name="q_Omega") for i in
                      range(self.n_Omega)]
        log_var_Omega = [tf.Variable(sigma2[i] * tf.ones([self.d_in[i], self.d_out[i]]), name="q_Omega") for i in
                         range(self.n_Omega)]
    ...
    def get_kl_Omega_to_learn(self):
        kl = 0
        for i in range(self.n_Omega):
            kl = kl + utils.DKL_gaussian(self.mean_Omega[i], self.log_var_Omega[i], self.prior_mean_Omega[i],
                                         self.log_prior_var_Omega[i])
        for i in range(self.n_W):
            kl = kl + utils.DKL_gaussian(self.mean_W[i], self.log_var_W[i], self.prior_mean_W[i],
    ...
    def sample_from_Omega_fixed(self):
        ...
        for i in range(self.n_Omega):
            ...
            if self.is_ard == True:
                reshaped_log_prior_var_Omega = tf.tile(tf.reshape(self.log_prior_var_Omega[i] / 2, [self.d_in[i], 1]),
                                                       [1, self.d_out[i]])
                Omega_from_q.append(tf.multiply(z, tf.exp(reshaped_log_prior_var_Omega)))
            if self.is_ard == False:
                Omega_from_q.append(
                    tf.add(tf.multiply(z, tf.exp(self.log_prior_var_Omega[i] / 2)), self.prior_mean_Omega[i]))
    ...
    def get_ell(self):
        ...
        layer = []
        layer.append(tf.multiply(tf.ones([self.mc, batch_size, Din]), X))

        ## Forward propagate information from the input to the output through hidden layers
        Omega_from_q = self.sample_from_Omega()

        for i in range(N_L):
            layer_times_Omega = tf.matmul(layer[i], Omega_from_q[i])  # X * Omega

            ## Apply the activation function corresponding to the chosen kernel - PHI
            if self.kernel_type == "RBF":
                Phi = tf.exp(0.5 * self.log_theta_sigma2[i]) / tf.cast(tf.sqrt(1. * self.n_rff[i]),
                                                                       'float32') * tf.concat(
                    values=[tf.cos(layer_times_Omega), tf.sin(layer_times_Omega)], axis=2)
            if self.kernel_type == "arccosine":
                if self.arccosine_degree == 0:
                    Phi = tf.exp(0.5 * self.log_theta_sigma2[i]) / tf.cast(tf.sqrt(1. * self.n_rff[i]),
                                                                           'float32') * tf.concat(
                        values=[tf.sign(tf.maximum(layer_times_Omega, 0.0))], axis=2)
                if self.arccosine_degree == 1:
                    Phi = tf.exp(0.5 * self.log_theta_sigma2[i]) / tf.cast(tf.sqrt(1. * self.n_rff[i]),
                                                                           'float32') * tf.concat(
                        values=[tf.maximum(layer_times_Omega, 0.0)], axis=2)
                if self.arccosine_degree == 2:
                    Phi = tf.exp(0.5 * self.log_theta_sigma2[i]) / tf.cast(tf.sqrt(1. * self.n_rff[i]),
                                                                           'float32') * tf.concat(
                        values=[tf.square(tf.maximum(layer_times_Omega, 0.0))], axis=2)
            ...
            if self.feed_forward and not (i == (
                    N_L - 1)):  ## In the feed-forward case, no concatenation in the last layer so that F has the same dimensions of Y
                F = tf.concat(values=[F, layer[0]], axis=2)
        ...
        return ell, layer_out, layer, Omega_from_q

    ## Maximize variational lower bound --> minimize Nelbo
    def get_nelbo(self):
        kl = self.get_kl()
        ell, layer_out, layer, Omega_from_q = self.get_ell()
        nelbo = kl - ell
        return nelbo, kl, ell, layer_out, layer, Omega_from_q
    ...
    def predict(self, data, mc_test):
        ...
        nll = - tf.reduce_sum(
            -np.log(mc_test) + utils.logsumexp(self.likelihood.log_cond_prob(self.Y, self.layer_out), 0))
        # nll = - tf.reduce_sum(tf.reduce_mean(self.likelihood.log_cond_prob(self.Y, self.layer_out), 0))
        pred, neg_ll, layer, Omega_from_q = self.session.run([out, nll, self.layer, self.omega_from_q], feed_dict={self.X: data.X, self.Y: data.Y, self.mc: mc_test})
        mean_pred = np.mean(pred, 0)
        return mean_pred, neg_ll, layer, Omega_from_q
    ...
    def get_vars_fixing_some(self, all_variables):
        if (self.q_Omega_fixed_flag == True) and (self.theta_fixed_flag == True):
            variational_parameters = [v for v in all_variables if
                                      (not v.name.startswith("q_Omega") and not v.name.startswith("log_theta"))]
    ...
    def learn(self, data, learning_rate, mc_train, batch_size, n_iterations, optimizer=None, display_step=100,
              test=None, mc_test=None, loss_function=None, duration=1000000, less_prints=False, dataset=None):
        total_train_time = 0
        ...
        print("Variational inference starts.")

        if not(less_prints):
            nelbo, kl, ell, _, _, _ =  self.session.run(self.get_nelbo(), feed_dict={self.X: data.X, self.Y: data.Y, self.mc: mc_train})
            print("Initial : kl=" + repr(kl) + "  nell=" + repr(-ell) + "  nelbo=" + repr(nelbo))
        ...
        for iteration in range(n_iterations):
            ...
            self.session.run(train_step,
                             feed_dict={self.X: batch[0], self.Y: batch[1], self.mc: monte_carlo_sample_train})
            ...
            if iteration % display_step == 0 or iteration == n_iterations - 1:
                ...
                if less_prints:
                    ...
                    nelbo, kl, ell, _, _, _ = self.session.run(self.get_nelbo(),
                    ...
                    # print(" W=", self.session.run(self.mean_W[0]), end=" ")

                if loss_function is not None:
                    pred, nll_test, _, _ = self.predict(test, mc_test)
                    elapsed_time = total_train_time + (current_milli_time() - start_predict_time)
                    loss_eval = loss_function.eval(test.Y, pred)
                    mnll_test = nll_test / len(test.Y)
                    ...
                    if loss_eval < min_loss:
                        ...
                        # log_lengthscale = self.session.run(self.log_theta_lengthscale)

                        ## save results
                        path = "../Initialization/" + dataset
                        if not os.path.exists(path):
                            os.mkdir(path)

                        for i in range(self.n_Omega):
                            mean_Omega = self.session.run(self.mean_Omega[i])
                            log_var_Omega = self.session.run(self.log_var_Omega[i])
                            np.savetxt("../Initialization/" + dataset + "/mean_Omega_" + str(i), mean_Omega)
                            np.savetxt("../Initialization/" + dataset + "/log_var_Omega_" + str(i), log_var_Omega)

                        for i in range(self.n_W):
                            mean_W = self.session.run(self.mean_W[i])
                            log_var_W = self.session.run(self.log_var_W[i])
                            np.savetxt("../Initialization/" + dataset + "/mean_W_" + str(i), mean_W)
                            np.savetxt("../Initialization/" + dataset + "/log_var_W_" + str(i), log_var_W)

                        hidden_layer, omega_from_q = self.session.run([self.layer, self.omega_from_q], feed_dict={self.X: data.X[:max(self.dhat_in),:], self.Y: data.Y[:max(self.dhat_in),:],self.mc: mc_test})
                        for i in range(1, self.nl):
                            for j in range(mc_test):
                                np.savetxt("../Initialization/" + dataset + "/hidden_layer_" + str(i) + "_" + str(j), hidden_layer[i][j,:,:])

                        for i in range(self.n_Omega):
                            for j in range(mc_test):
                                np.savetxt("../Initialization/" + dataset + "/Omega_" + str(i) + "_" + str(j), omega_from_q[i][j,:,:])

                        log_theta_sigma2 = self.session.run(self.log_theta_sigma2)
                        np.savetxt("../Initialization/" + dataset + "/log_theta_sigma2", log_theta_sigma2)

                        if self.likelihood.get_task() == "Regression":
                            log_var = self.session.run(self.likelihood.log_var)
                            np.savetxt("../Initialization/" + dataset + "/log_var", np.array([[log_var]]))

                        np.savetxt("../Initialization/" + dataset + "/loss", np.array([[min_loss]]))
                        np.savetxt("../Initialization/" + dataset + "/MNLL", np.array([[min_mnll]]))

                print(" time=" + repr(elapsed_time), end=" ")
                print("")

        print("Variational inference ends. The number of best iteration is", num_iteration)
        print("")
